...
//BatchedGeometry.h
//A "lightweight" version of StaticGeometry, which gives you a little more control
...
void BatchedGeometry::addEntity(Entity *ent, const Vector3 &position, const Quaternion &orientation, const Vector3 &scale, const ColourValue &color)
{
...
uint32 CountUsedVertices(IndexData *id, std::map<uint32, uint32> &ibmap)
{
	...
	switch (id->indexBuffer->getType()) {
		...
		default:
			throw new Exception(0, "Unknown index buffer type", "Converter.cpp::CountVertices");
			break;
	}
...
void BatchedGeometry::extractVertexDataFromShared(MeshPtr mesh)
{
	...
	CORE_DELETE(mesh->sharedVertexData);
}
...
void BatchedGeometry::clear()
{
	...
	//Delete each batch
	for(auto i = subBatchMap.begin(); i != subBatchMap.end(); ++i)
		delete i->second;
...
void BatchedGeometry::_notifyCurrentCamera(Camera *cam)
{
	...
	else
	{
		...
		f32 centerDistanceSquared = camVec.squaredLength();
...
Vector3 BatchedGeometry::_convertToLocal(const Vector3 &globalVec) const
{
...
{
	clear();
	CORE_DELETE(vertexData);
	CORE_DELETE(indexData);
}
...
void BatchedGeometry::SubBatch::addSubEntity(SubEntity *ent, const Vector3 &position, const Quaternion &orientation, const Vector3 &scale, const ColourValue &color, void* userData)
{
	assert(!built);
...
void BatchedGeometry::SubBatch::build()
{
	...
	for (ushort i = 0; i < vertBinding->getBufferCount(); ++i)
	{
	...
	if (requireVertexColors) {
		if (!vertexData->vertexDeclaration->findElementBySemantic(VES_DIFFUSE)) {
			ushort i = (ushort)vertBinding->getBufferCount();
	...
	for (it = meshQueue.begin(); it != meshQueue.end(); ++it) {
		...
		for (ushort i = 0; i < destBinds->getBufferCount(); ++i)
		{
...
void BatchedGeometry::SubBatch::setMaterial(MaterialPtr &mat) 
{ 
	material = mat;
}

void BatchedGeometry::SubBatch::setMaterialName(const String &mat) 
{ 
	material = MaterialManager::getSingleton().getByName(mat); 
}

inline String BatchedGeometry::SubBatch::getMaterialName() const 
{ 
...
f32 BatchedGeometry::SubBatch::getSquaredViewDepth(const Camera* cam) const
{
...
const LightList& BatchedGeometry::SubBatch::getLights(void) const
{
...
Technique* BatchedGeometry::SubBatch::getTechnique() const 
{ 
	return bestTechnique; 
}

const MaterialPtr& BatchedGeometry::SubBatch::getMaterial(void) const 
{ 
	return material;
}

void BatchedGeometry::SubBatch::getWorldTransforms(Matrix4* xform) const 
{ 
	*xform = parent->_getParentNodeFullTransform(); 
}

const Quaternion& BatchedGeometry::SubBatch::getWorldOrientation(void) const
{ 
	return parent->sceneNode->_getDerivedOrientation();
}

const Vector3& BatchedGeometry::SubBatch::getWorldPosition(void) const
{ 
	return parent->sceneNode->_getDerivedPosition();
}

bool BatchedGeometry::SubBatch::castsShadows(void) const
{ 
