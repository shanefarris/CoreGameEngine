...
void BatchPage::setFade(bool enabled, f32 visibleDist, f32 invisibleDist)
{
	if (!shadersSupported)
		return;
...
const Ogre::AxisAlignedBox& BatchPage::getBoundingBox() 
{ 
...
void BatchPage::_updateShaders()
{
	...
	while (it.hasMoreElements()){
		...
		if (lightingEnabled)
			tmpName << "lit_";
		if (subBatch->vertexData->vertexDeclaration->findElementBySemantic(VES_DIFFUSE) != nullptr)
			tmpName << "clr_";
		...
		if (HighLevelGpuProgramManager::getSingleton().getByName(vertexProgName).isNull())
		{
			...
			if(!shaderLanguage.compare("hlsl") || !shaderLanguage.compare("cg"))
			{
				...
				if (subBatch->vertexData->vertexDeclaration->findElementBySemantic(VES_DIFFUSE) != nullptr) vertexProgSource +=
					"	float4 iColor    : COLOR, \n";
...
String BatchPage::getUniqueID(const String &prefix)
{
	return prefix + StringConverter::toString(++GUID);
}
