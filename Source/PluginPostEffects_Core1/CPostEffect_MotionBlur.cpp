...
CPostEffect_MotionBlur::CPostEffect_MotionBlur() : CPostEffect("MOTION_BLUR")
{
...
void CPostEffect_MotionBlur::RegisterPostEffect()
{
	CompositorManager::getSingleton().addCompositor(m_Viewport, mCompName);
}

void CPostEffect_MotionBlur::notifyMaterialRender(u32 pass_id, MaterialPtr& mat)
{
	// Just return if it's not CMotionBlur
	if (pass_id != 42)
		return;
