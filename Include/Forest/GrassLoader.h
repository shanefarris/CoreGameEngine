...
namespace Forests
{
	...
	*/
	class CORE_EXPORT GrassLoader: public PageLoader
	{
	public:
		...
		GrassLayer *addLayer(const String &material);
		...
		*/
		inline void setWindDirection(Vector3 &dir) { windDir = dir; }
		...
		inline Vector3 &getWindDirection() { return windDir; }
		...
		*/
		...
		to take effect.	*/
		inline void setRenderQueueGroup(uint8 queueID) { renderQueue = queueID; }
		...
		*/
		void setHeightFunction(Real (*heightFunction)(Real x, Real z, void *userData), void *userData = nullptr) {
	...
	private:
		...
		//Helper functions
		Mesh *generateGrass_QUAD(PageInfo &page, GrassLayer *layer, float *grassPositions, unsigned int grassCount);
		Mesh *generateGrass_CROSSQUADS(PageInfo &page, GrassLayer *layer, float *grassPositions, unsigned int grassCount);
		Mesh *generateGrass_SPRITE(PageInfo &page, GrassLayer *layer, float *grassPositions, unsigned int grassCount);

		//List of grass types
		std::list<GrassLayer*> layerList;

		//Height data
		Real (*heightFunction)(Real x, Real z, void *userData);	//Pointer to height function
		...
		{
			return "GrassLDR" + StringConverter::toString(++GUID);
		}
	};
	...
	*/
	...
	public:
		...
		void setMaxSlope(Radian maxSlopeAngle) {
			if (maxSlopeAngle > Degree(89.99f))
				maxSlopeAngle = Degree(89.99f);
			if (maxSlopeAngle < Degree(0))
				maxSlopeAngle = Degree(0);

			maxSlope = Math::Tan(maxSlopeAngle);
		}
		...
		want. */
		void setDensityMap(const String &mapFile);
		void setDensityMap(const String &mapFile, MapChannel channel);
		...
		void setDensityMap(TexturePtr map);
		void setDensityMap(TexturePtr map, MapChannel channel);
		...
		*/
		...
		so the density/color map is aligned to your heightmap, but you could apply it anywhere you
		want. */
		void setColorMap(const String &mapFile);
		void setColorMap(const String &mapFile, MapChannel channel);
		...
		void setColorMap(TexturePtr map);
		void setColorMap(TexturePtr map, MapChannel channel);
	...
	*/
	class CORE_EXPORT GrassPage: public GeometryPage
	{
	public:
		void init(PagedGeometry *geom, const Any &data);
		~GrassPage();

		void addEntity(Entity *ent, const Vector3 &position, const Quaternion &rotation, const Vector3 &scale, const ColourValue &color);
		void removeEntities();
		void setFade(bool enabled, Real visibleDist, Real invisibleDist) {}
	...
	private:
		...
		inline String getUniqueID()
		{
			return "GrassPage" + StringConverter::toString(++GUID);
		}
	};
